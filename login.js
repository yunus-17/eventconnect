...
// API Base URL (fallback to localhost when opened as a file or from another origin)
const API_BASE_URL = (typeof window !== 'undefined' && window.location && typeof window.location.origin === 'string' && window.location.origin.startsWith('http'))
...
async function handleLogin(type, data) {
  try {
    ...
    if (result.success) {
      ...
      // Determine redirect target with explicit role-based override
      let redirectTo = result.redirectTo;
      if (result.user && result.user.role === 'admin') {
        redirectTo = '/eventconnect-admin.html';
      } else if (result.user && result.user.role === 'student') {
